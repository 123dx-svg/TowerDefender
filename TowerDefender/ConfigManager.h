...
public:
	...
	{
		std::string window_title = u8"守卫村子";
...
public:
				...
				cJSON_ArrayForEach(json_spawn_event, json_wave_spawn_list)
				{
					...
					if (json_spawn_event_enemy_type && json_spawn_event_enemy_type->type == cJSON_String)
					{
						...
						if (str_enemy_type == "Slim")
							spawn_event.enemy_type =EEnemyType::Slim;
						else if (str_enemy_type == "KingSlim")
							spawn_event.enemy_type = EEnemyType::KingSlim;
						else if (str_enemy_type == "Skeleton")
							spawn_event.enemy_type = EEnemyType::Skeleton;
						else if (str_enemy_type == "Goblin")
							spawn_event.enemy_type = EEnemyType::Goblin;
						else if (str_enemy_type == "GoblinPriest")
							spawn_event.enemy_type = EEnemyType::GoblinPriest;
	...
	bool LoaGameConfig(const std::string& path)
	{
		std::ifstream file(path);
		if (!file.good())return false;
...
private:
	...
	//解析"damage":	[10, 12, 14, 16, 18, 20, 22, 24, 25, 25],
	void parse_number_array(double* ary, int max_len, cJSON* json_root)
	{
		...
		{
			idx++;
			if (json_item->type != cJSON_Number||idx>=max_len) continue;
			//数组传入
			ary[idx] = json_item->valuedouble;
